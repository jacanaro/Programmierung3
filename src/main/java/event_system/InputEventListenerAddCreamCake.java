...
public class InputEventListenerAddCreamCake implements InputEventListener {
    @Override
                ...
                if (userInputStrArr.length > 2) {
                    for (int i = 0; i < userInputStrArr.length; i++) {
                        if (userInputStrArr[i].equals("Kremkuchen")) {
                            ...
                            if (cli.getLog() != null)
                                cli.getLog().logger.info("es wird versucht, dem Automat einen Kremkuchen hinzuzufügen");
                            ...
                            if (vendingMachineErrorCodes == null && cli.getLog() != null)
                                cli.getLog().logger.info("Kremkuchen wurde hinzugefügt");
                        }
                    }
                }
            }
